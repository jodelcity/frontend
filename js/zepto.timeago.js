...
(function($){
  ...
  $.fn.timedata = function(key, value) {
...
(function($) {
  ...
  function prepareData(element) {
    element = $(element);
    if (!element.timedata("timeago")) {
      element.timedata("timeago", { datetime: $t.datetime(element) });
      var text = $.trim(element.text());
      if (text.length > 0) {
        element.attr("title", text);
      }
    }
    return element.timedata("timeago");
  }
